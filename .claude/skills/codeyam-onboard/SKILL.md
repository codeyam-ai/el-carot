...
1. ` editor health-status --format json` returns `passed: true`.
...
```
 editor migration-status --format json
```
...
3. **Translate the archived legacy scenarios (phase 2b).** The legacy `editor-scenarios/` files use UUID filenames + `_metadata` wrappers, a different shape than the new schema. Translate them in place rather than asking the user to re-register hundreds of scenarios by hand:
   ```
    editor translate-legacy-scenarios --dry-run
   ```
   ...
   ```
    editor translate-legacy-browser-state
   ```
...
```
 editor migrate-journal --format json
```
...
**Capability check (before you write `apps[]`).** Once you've detected the framework, run ` editor stack-support --framework <detected> --format json` (add `--app-type backend|cli` for non-web projects). Relay the verdict to the user before writing editor.json:
...
2. **Glossary.** Required by the App tab's components/functions enumeration and by the audit gate. Auto-applies the unambiguous adds (entities with `///`/JSDoc + an auto-derivable test file); the rest land as skips with reasons.
   ...
   ```
    editor reconcile-glossary --auto-apply
   ```
...
2. For each, register it. Component scenarios use isolation routes; application scenarios use real URLs. The exact invocation is:
   ...
   EOF
    editor register --file .codeyam/tmp/register-home-<unique>.json
   ```
...
3. If the dev server is running, attempt capture via ` editor recapture-stale --skip-when-clean`. If not running, skip and note it.
...
```
 editor health-status --format json
```
...
- **Smoke layer**
  ...
  - `clean_settings` failed → `.claude/settings.local.json` carries dead skill permissions or stale hook entries from a prior tool version. Run ` editor scrub-settings` (dry-run) to see findings; if everything reported is safe to remove (dead skills, stale hooks — never plaintext credentials), ask the user, then run ` editor scrub-settings --apply`. Plaintext-credential lookalikes flagged by scrub-settings are **never** auto-removable — surface them in the report and tell the user.
...
If `$HOME/.codeyam/notify.env` does not exist, suggest ` editor notify-setup --topic <claude-$(whoami)-$(openssl rand -hex 4)>` once and continue. Don't block onboarding on it — the notify hooks are pre-installed and silently no-op until the developer opts in.
