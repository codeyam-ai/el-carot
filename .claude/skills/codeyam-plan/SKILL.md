...
The one read-only CLI call this skill makes is ` editor last-plan-prefix` in Step 2 (to suggest the most recent prefix). It prints to stdout and changes nothing — it is not an "implementation" command.
...
1. Run ` editor last-plan-prefix` and capture its trimmed stdout as `lastPrefix`. It prints the prefix of the single most-recently-created plan (scanning both the queue and `.codeyam/plans/completed/`), or nothing when that newest plan has no prefix — or there are no plans yet.
...
- ` editor glossary-find <name>` (flags: `--prefix`,
  `--substring`, `--feature`, `--format`) — look up named entries
- ` editor glossary-list` / `glossary-untested` /
...
```bash
 editor classify-constrained-files <path>... --format json
```
...
Run ` editor plans` to verify the plan is parseable and shows up correctly.
