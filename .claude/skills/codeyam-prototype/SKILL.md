...
- Register scenarios on the fly with ` editor register` so
...
- Do **NOT** run ` editor advance` or ` editor
...
- **Every batch of changes ends with a `preview-nav`.** After you finish
  ...
  ` editor preview-nav` pointing the iframe at the
...
- **`preview-nav` is the in-loop iteration tool.** It's lightweight
  ...
  Use the heavier ` editor preview` only when you need
...
- **Register scenarios before navigating to them.** `preview-nav` with
  ...
  ` editor register` first, then navigate.
...
> Once the Write succeeds, run ` editor launch-plan <slug>` to
...
4. After the Write succeeds, run ` editor launch-plan <slug>`
...
- ` editor advance` / `step` — those belong to Build.
