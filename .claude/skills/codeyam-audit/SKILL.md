...
description: |
  ...
  ` editor session-finalize`, and on failure groups the
...
Run ` editor session-finalize` on the current branch and surface deferred-commit attribution on failure.
...
```bash
 editor config-show >/dev/null 2>&1 || {
...
Run ` editor finalize-debt show --format json` and parse the result. The JSON shape is:
...
```bash
 editor session-finalize 2>&1 | tee /tmp/codeyam-audit-finalize.log
```
...
**On failure** (non-zero exit): re-run ` editor audit --format json` to get the structured failure list with the new `attribution[]` field. The shape:
