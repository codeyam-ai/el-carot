...
```
 editor changed-surfaces --base <ref> --format json
```
...
```
 editor preview-nav '{"path":"/","dimension":"<defaultScreenSize>"}'
```
...
EOF
 editor register --file .codeyam/tmp/recent-change-<suggestedSlug>-<unique>.json
```
...
```
 editor recapture-stale --skip-when-clean
```
